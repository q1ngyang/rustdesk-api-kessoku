...
type ServerControlProvider interface {
	...
	Relays(context.Context) ([]Relay, error)
	...
	ApplyConfig(context.Context, ApplyRequest) (ApplyResult, error)
	Operation(context.Context, string) (Operation, error)
	ConfigHistory(context.Context) ([]ConfigRevision, error)
	RollbackConfig(context.Context, RollbackRequest) (ApplyResult, error)
}
