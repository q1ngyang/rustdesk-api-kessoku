...
[English](RELEASE-NOTES-v2.8.3.md) | **简体中文**

> v2.8.3 替代未发布的 `v2.8.0` 源码 tag。其候选工作流因容器 shell 配置错误在构建前
...
>
> 后续 `v2.8.2` 源码 tag 也是未发布且不可变的 tombstone。完整源码/runtime 门禁已通过，
> 但 GitHub artifact 解包规范化可执行位后，下游打包按预期失败关闭；没有创建最终候选、
> 软件包、镜像、`latest` 更新、Release 或已发布资产。

v2.8.3 将原通用 RustDesk API 服务收紧为有明确边界的账户与管理控制面，并面向
...
- v2.8.3 正式支持 Linux amd64：GHCR 镜像、Linux x86_64 archive/binary 与 amd64 DEB。
...
- 外部 MySQL 现在要求 `mysql.tls: "true"`；可用 `mysql.ca-file` 把私有 CA 加入系统信任
  ...
  不验证主机名的数据库传输会导致启动失败。
- 迁移是增量的，但旧应用不能使用不含明文 token 的新凭据。v2.8.3 一旦签发令牌，回滚
...
- 在发布不可变 tag、GHCR `v2.8.3`/`latest`、GitHub Release 与 Wiki 前，对该批准提交运行
  受保护的非发布候选 CI。
