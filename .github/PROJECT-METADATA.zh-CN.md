...
本文记录发布负责人已批准的 v3.0.7 外部 GitHub 元数据；远端变更仍只能按受保护发布流程执行。
...
发布流程会把不可变 `v3.0.7` 与移动的 `latest` 发布为同一镜像。`latest` 指向最新成功
...
- 在 `master` 构建精确的 v3.0.7 candidate，完成发布就绪验证后才创建不可变 tag；
...
- 把一个带 OCI provenance 与 SBOM 的 linux/amd64 GHCR 镜像同时发布为 `v3.0.7`
  与 `latest`；
