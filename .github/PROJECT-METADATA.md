...
v2.8.1. Remote changes remain restricted to the protected release process.
...
> Kessoku v2.8.1 RustDesk account and administration API with EdDSA token
...
The release publishes immutable `v2.8.1` and moving `latest` tags for the same
...
- publish the exact successful, non-publishing v2.8.1 candidate;
...
- publish one linux/amd64 GHCR image under both `v2.8.1` and `latest`, with OCI
