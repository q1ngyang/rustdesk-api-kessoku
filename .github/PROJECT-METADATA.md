...
v3.0.3. Remote changes remain restricted to the protected release process.
...
> Kessoku v3.0.3 RustDesk account and enterprise administration API with
...
The release publishes immutable `v3.0.3` and moving `latest` tags for the same
...
- publish the exact successful, non-publishing v3.0.3 candidate;
...
- publish one linux/amd64 GHCR image under both `v3.0.3` and `latest`, with OCI
