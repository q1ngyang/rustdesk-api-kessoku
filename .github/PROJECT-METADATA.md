...
v3.0.2. Remote changes remain restricted to the protected release process.
...
The release publishes immutable `v3.0.2` and moving `latest` tags for the same
...
- publish the exact successful, non-publishing v3.0.2 candidate;
...
- publish one linux/amd64 GHCR image under both `v3.0.2` and `latest`, with OCI
