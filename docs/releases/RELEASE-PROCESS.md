...
`RELEASE_STATUS` records the release owner's approval of `v3.0.2`. Approval
...
2. Create immutable tag `v3.0.2` at that commit; never move or reuse it.
...
6. After the protected environment gate, verify the GitHub Release, asset
   checksums and attestations, GHCR `v3.0.2`, and `latest`; both image tags must
...
8. Only after v3.0.2 is verified, remove the withdrawn v3.0.1 package version.
...
[`MIGRATION-v3.0.2.md`](v3.0.2/MIGRATION-v3.0.2.md) and restore the complete
