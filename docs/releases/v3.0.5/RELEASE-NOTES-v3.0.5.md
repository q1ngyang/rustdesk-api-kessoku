...
v3.0.5 is a stability and operations release for the supported Kessoku v3
...
The unpublished `v3.0.4` tag is retained as a failed-candidate audit record.
v3.0.5 contains the same reviewed product scope, synchronized generated API
documentation, and a redesigned release sequence that completes the full
candidate and publication-readiness gates before creating an immutable tag.
...
- Moved complete artifact, database, frontend, documentation, attestation,
  environment, and image-path validation ahead of immutable tag creation.
  Future candidate failures therefore do not consume patch-version numbers.
...
- Back up the database, TOTP key, media directory, configuration, signing keys,
  and PKI before upgrading. Start only one v3.0.5 writer during the migration.
...
See the [v3.0.5 migration and rollback guide](MIGRATION-v3.0.5.md) and the
[operations guide](https://github.com/q1ngyang/rustdesk-api-kessoku/wiki/Operations-and-Verification).

[简体中文](RELEASE-NOTES-v3.0.5.zh-CN.md)
