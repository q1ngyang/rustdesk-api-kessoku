# RustDesk API Kessoku v3.0.4 migration and rollback

v3.0.4 keeps database version `309` and uses the normal startup AutoMigrate to
...
v3.0.4 against the same database during migration.

## Upgrade

1. Restore the backup into an isolated rehearsal environment.
2. Start one v3.0.4 instance and wait for schema migration to finish.
...
6. Repeat the stopped-writer backup and upgrade sequence in production, then
   pin the verified v3.0.4 image digest.

## Rollback

Do not point v3.0.3 or an older process at a database already opened by v3.0.4.
...
[简体中文](MIGRATION-v3.0.4.zh-CN.md)
