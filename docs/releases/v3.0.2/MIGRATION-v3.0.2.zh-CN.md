# RustDesk API Kessoku v3.0.2 数据库迁移与回退

v3.0.2 会将 Kessoku 数据库依次升级到版本 `309`。迁移主要为增量变更，但会引入持久化
...
1. 在隔离环境恢复备份并只启动一个 v3.0.2 实例，禁止新旧版本并发写入。
...
不得让 v3.0.1、v3.0.0 或任何 v2 镜像读取已经迁移到 309 的数据库。停止全部 v3.0.2
