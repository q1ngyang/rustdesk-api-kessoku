...
> 在受保护发布流程完成前，本文中的 `v2.8.1` 镜像仍只是发布目标。正式发布的 `latest`
...
v2.8.1 镜像包含一个非特权 `kessoku-api` 进程、从同一源码提交构建的已审核管理前端与
...
发布 workflow 会把不可变 `v2.8.1` 与移动的 `latest` 推送为同一镜像。只有在明确希望跟随
最新稳定版时才使用 `latest`；生产变更控制与回滚应解析并固定版本 tag 的 digest。

## Compose 快速开始

在 v2.8.1 源码或下载的部署文件目录中：
...
Kessoku 数据库版本 301 是增量迁移，但旧程序无法认证 v2.8.1 新签发的仅 hash token。
如果 v2.8.1 已签发令牌，回滚旧程序时必须恢复匹配的升级前数据库备份。
