...
From Kessoku v2.8.2 onward, this directory and the backend are one versioned source unit. Candidate provenance therefore records the Kessoku commit itself plus this seed/import lineage; no independent frontend repository, branch, tag, or network checkout is a build input. The root candidate workflow rebuilds from `admin-web/package-lock.json` with `npm ci`, and runtime packaging copies only the resulting `dist/` into `resources/admin`.
