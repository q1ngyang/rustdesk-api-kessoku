...
本目录是 `rustdesk-api-kessoku` v3 开发线的内置 Vue 3 管理前端源码，直接延续本仓库 v2.8.3 已整合并调整过的前端。它与 Go 后端在同一提交中审核、测试和发布，不依赖独立前端仓库、外部前端分支或 `lejianwen/rustdesk-api-web`。

初始 v3 界面设计、响应式行为、企业管理员权限边界和改造理由见历史文档 [UI-REDESIGN-v3.0.1.zh-CN.md](../docs/development/UI-REDESIGN-v3.0.1.zh-CN.md)。v3.0.3 及后续版本在此基础上增加集中式品牌、TOTP、头像、GeoIP、多语言和独立 WebClient；权限部分继续采用 `user/admin/super_admin` 三层角色和服务端资源范围校验，不改变 RustDesk/Starry 安全协议。
