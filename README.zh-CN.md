...
Kessoku 是一个非官方的 RustDesk 账户、管理和策略控制面。它提供客户端 API 与内置管理
前端，并通过类型化、版本化 Control API 与
...
> 契约也已固定，已发布 Starry 的 audit→enforce 真实客户端矩阵在本机通过；仍需完成
...
项目不包含、不下载、不代理 WebClient2，也不提供许可证绕过功能。
...
Compose 默认把应用端口绑定到 `127.0.0.1`，请通过已审核的 HTTPS 反代范例
