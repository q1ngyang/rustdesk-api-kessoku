...
printf '%s\n' 'v2.8.1-local-candidate' > "$release/resources/version"
cp -a "$backend_source/conf" "$backend_source/docs" "$release/"
for document in README.md README.zh-CN.md README_EN.md CONTAINER.md \
  CONTAINER.zh-CN.md RELEASE-NOTES-v2.8.1.md \
  RELEASE-NOTES-v2.8.1.zh-CN.md SECURITY-MODEL.md MIGRATION.md \
...
sh "$backend_source/scripts/build-deb.sh" "$release" \
  '2.8.1~local.1-1' amd64 "$candidate_root/packages-a"
sh "$backend_source/scripts/build-deb.sh" "$release" \
  '2.8.1~local.1-1' amd64 "$candidate_root/packages-b"
...
docker run --rm --platform linux/amd64 \
  ...
  "$debian_test_image" sh -euxc '
    ...
    test "$(dpkg-query -W -f=\${Version} kessoku-api)" = "2.8.1~local.1-1"
...
install -m 0644 "$candidate_root/candidate-a.tar.gz" \
  "$release_assets/kessoku-v2.8.1-local-linux-amd64.tar.gz"
...
install -m 0644 "$backend_source/RELEASE_STATUS" \
  ...
  "$backend_source/RELEASE-NOTES-v2.8.1.md" \
  "$backend_source/RELEASE-NOTES-v2.8.1.zh-CN.md" \
