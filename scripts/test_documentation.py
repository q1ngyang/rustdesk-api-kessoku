...
class DocumentationLinksTest(unittest.TestCase):
    def setUp(self):
        ...
        self.source = self.root / "docs/releases/v3.0.2/RELEASE-NOTES-v3.0.2.md"
        for path in (
            "docs/wiki/Home.md", "docs/wiki/ZH-CN-Home.md",
            "docs/releases/v3.0.2/RELEASE-NOTES-v3.0.2.md",
            "docs/releases/v3.0.2/MIGRATION-v3.0.2.md",
    ...
    def test_renderer_rebases_nested_release_link_and_preserves_label(self):
        body = "[MIGRATION-v3.0.2.md](MIGRATION-v3.0.2.md#upgrade)\n"
        rendered = render(body, self.source, REPOSITORY, "v3.0.2", self.root)
        self.assertEqual(rendered, f"[MIGRATION-v3.0.2.md]({self.base}/blob/v3.0.2/docs/releases/v3.0.2/MIGRATION-v3.0.2.md#upgrade)\n")
    ...
    def test_renderer_uses_wiki_and_tree_urls(self):
        ...
        self.assertIn(f"]({self.base}/tree/v3.0.2/examples/relay)", rendered)

    def test_renderer_preserves_external_anchor_and_code(self):
        body = "[external](https://example.com/) [here](#section)\n```md\n[x](missing.md)\n```\n"
        self.assertEqual(render(body, self.source, REPOSITORY, "v3.0.2", self.root), body)

    def test_renderer_rejects_missing_target(self):
        with self.assertRaises(ValueError):
            render("[x](missing.md)", self.source, REPOSITORY, "v3.0.2", self.root)
