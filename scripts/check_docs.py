...
def paired_documents() -> list[tuple[Path, Path]]:
    pairs = [
        ...
        (
            ROOT / "RELEASE-NOTES-v2.8.1.md",
            ROOT / "RELEASE-NOTES-v2.8.1.zh-CN.md",
        ),
        (
...
def main() -> int:
    ...
    for required in (
        ...
        if "v2.8.1" not in required.read_text(encoding="utf-8"):
            errors.append(f"v2.8.1 is missing from {required.relative_to(ROOT)}")
